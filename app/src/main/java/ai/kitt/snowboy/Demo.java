...
public class Demo extends Activity implements KeywordSpottingCallback {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        KeywordSpottingManager.sharedInstance.setup(this, this);
        activeTimes = 0;
        playbackThread = new PlaybackThread(this);

        try {
            String basePath = getFilesDir().getAbsolutePath() + Constants.DEFAULT_WORK_SPACE;
            player.setDataSource(basePath + "ding.wav");
            player.prepare();
        } catch (IOException e) {
    ...
    private void startRecording() {
        KeywordSpottingManager.sharedInstance.start();
        updateLog(" ----> recording started ...", "green");
        record_button.setText(R.string.btn1_stop);
    }

    private void stopRecording() {
        KeywordSpottingManager.sharedInstance.stop();
    ...
    public void updateLog(final String text) {
        ...
        logView.post(() -> logView.fullScroll(ScrollView.FOCUS_DOWN));
    }
    ...
    @Override
    public void onListening(boolean result) {
